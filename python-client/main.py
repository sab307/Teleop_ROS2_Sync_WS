...
class P2PTwistClient:
    ...
    async def _send_ack_p2p(self, twist: TwistWithLatency):
        ...
        ts = LatencyTimestamps(
            # t1_browser_send intentionally omitted — it's no longer carried
            # on the ack wire (browser looks it up locally via msgId).
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Go signaling server URL (ws:// or wss://)"
    )
    p.add_argument(
        "--ca-cert",
        default=None,
...
async def main():
    ...
    # Expose --ca-cert via env var so _signaling_loop() picks it up
    if args.ca_cert:
        os.environ["TLS_CA"] = args.ca_cert

    tls_note = ""
    if args.signal.startswith("wss://"):
