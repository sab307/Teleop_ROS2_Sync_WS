...
export const state = {
    ...
    /** When true, velocity fields are packed as float32 instead of float64.
     *  Toggled by the precision button in the Field Selector panel. The flag
     *  is OR'd into the wire mask byte (bit 7) by encodeTwist(), so each
     *  message is self-describing — no handshake needed. */
    halfPrecision: false,

    /** Map<msgId, {t1, perfMs}> — pending outbound twists awaiting their ack.
     *  t1 is no longer echoed on the wire; the browser stores it locally and
     *  looks it up via msgId when the ack arrives. Entries are removed on ack
     *  and swept after 5 s to bound the map size if acks are lost. */
